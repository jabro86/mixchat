		...
		getOrCreateChannel: requiresAuth.createResolver(
			async (parent, { teamId, members }, { models, user }) => {
				...
				const [data] = await models.sequelize.query(
					"select c.id, c.name from channels as c, pcmembers as pc " +
						"where pc.channel_id = c.id and c.dm = true and " +
						"c.public = false and c.team_id = " +
						teamId +
						" group by c.id, c.name " +
						"having array_agg(pc.user_id) @> Array[" +
						allMembers.join(",") +
						"] and count(pc.user_id) = " +
						allMembers.length +
						";",
					{
						raw: true
